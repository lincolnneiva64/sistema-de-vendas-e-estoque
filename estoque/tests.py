...
class PixRecebidoTests(TestCase):
    ...
    def test_conferencia_recebimentos_rota_exibe_contagem_por_cedulas_e_direta(self):
        ...
        self.assertContains(resposta, "posicionarCalculadora")
        self.assertContains(resposta, 'painelConferenciaEl.getBoundingClientRect()')
        self.assertContains(resposta, 'window.addEventListener("resize", posicionarCalculadora)')
        self.assertContains(resposta, 'window.removeEventListener("resize", posicionarCalculadora)')
        self.assertContains(resposta, 'modalCalculadoraEl.classList.add("lateral")')
        self.assertContains(resposta, 'modalCalculadoraEl.classList.remove("lateral")')
    ...
    def test_conferencia_recebimentos_rota_exige_observacao_quando_ha_diferenca(self):
        ...
        self.assertEqual(resposta.status_code, 200)
        self.assertContains(resposta, "Informe uma observacao para finalizar com falta ou sobra.")
