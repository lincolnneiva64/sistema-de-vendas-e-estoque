...
class PixRecebidoTests(TestCase):
    ...
    def test_recibo_whatsapp_card_retorna_png_vertical_sem_alterar_status(self):
        ...
        self.assertEqual(imagem.width, 520)
        self.assertGreater(imagem.height, imagem.width)
        ...
        self.assertNotIn("SALDO ANTERIOR", textos_renderizados)
        self.assertNotIn("Contas abatidas", textos_renderizados)
        self.assertFalse(any(texto.startswith("Nota #") for texto in textos_renderizados))
        self.assertFalse(any("Abatido" in texto or "Restante" in texto or "Parcial" in texto for texto in textos_renderizados))
        operacao.refresh_from_db()
        self.assertEqual(operacao.status_recibo, OperacaoRecebimentoCliente.STATUS_RECIBO_PENDENTE)
